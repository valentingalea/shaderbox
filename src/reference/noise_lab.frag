...
float map(vec3 pos)
{
    return length(pos - (vec3(0.0, 0.0, 1.5) + iGlobalTime * vec3(0.0, 0.0, SPEED))) - 1.0;
}
...
{
    vec3 pos = vec3(uv * vec2(iResolution.x/iResolution.y, 1.0), iGlobalTime * SPEED);
    float f;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec3 ro = vec3(0.0, 0.0, 0.0) + iGlobalTime * vec3(0.0, 0.0, SPEED);
