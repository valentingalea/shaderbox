...
// made periodic with trick from here: https://www.shadertoy.com/view/4dlGW2
float pcnoise(_in(vec3) Pin, _in(float) scale)
{
	vec3 P = Pin * scale;

	vec3 Pi0 = floor(P); // Integer part for indexing
	vec3 Pi1 = Pi0 + vec3(1, 1, 1); // Integer part + 1
	Pi0 = mod289(mod(Pi0, scale));
	Pi1 = mod289(mod(Pi1, scale));
	vec3 Pf0 = fract(P); // Fractional part for interpolation
	vec3 Pf1 = Pf0 - vec3(1, 1, 1); // Fractional part - 1.0
	vec4 ix = vec4(Pi0.x, Pi1.x, Pi0.x, Pi1.x);
	vec4 iy = vec4(Pi0.yy, Pi1.yy);
...
float pnoise(_in(vec3) P, _in(vec3) rep)
{
